...
public class DiceRoll : MonoBehaviour {
	...
	Transform t;
	///<summary>type = "boss" ou "player", <para></para>
	...
	public void RollDices(string type, int diceNumber, string diceInfos, int testDif){
		...
		if(type=="player")StartCoroutine(StartRollPlayer(diceNumber,diceInfos,testDif));
		if(type=="boss")StartCoroutine(StartRollBoss(diceNumber,diceInfos,testDif));
	}

	public IEnumerator StartRollPlayer(int diceNumber, string diceInfos, int testDif)
	{
        catDiceposition = 7;
		catDices.Clear();
